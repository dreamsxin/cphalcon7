...
typedef struct  {
    phalcon_cache_memory_shared_segment common;
    int shm_id;
} phalcon_cache_memory_shared_segment_shm;

static int create_segments(size_t k_size, size_t v_size, phalcon_cache_memory_shared_segment_shm **shared_segments_p, int *shared_segments_count, char **error_in) /* {{{ */ {
	struct shmid_ds sds;
	int shm_id, shmget_flags;
	phalcon_cache_memory_shared_segment_shm *shared_segments, first_segment;
    unsigned int i, j, allocate_size, allocated_num, segments_num, segment_size;

	shmget_flags = IPC_CREAT|SHM_R|SHM_W|IPC_EXCL;
    segments_num = 1024;
    while ((v_size / segments_num) < PHALCON_CACHE_MEMORY_SMM_SEGMENT_MIN_SIZE) {
        segments_num >>= 1;
    }
    segment_size = v_size / segments_num;
    allocate_size = PHALCON_CACHE_MEMORY_SMM_SEGMENT_MAX_SIZE;
    ...
    if (allocate_size < PHALCON_CACHE_MEMORY_SMM_SEGMENT_MIN_SIZE) {
        /* this should never happen */
        *error_in = "shmget";
        return 0;
    }
    ...
    shared_segments = (phalcon_cache_memory_shared_segment_shm *)calloc(1, (allocated_num) * sizeof(phalcon_cache_memory_shared_segment_shm));
    if (!shared_segments) {
        *error_in = "calloc";
        return 0;
    }
    ...
    ++segments_num;
    *shared_segments_p = (phalcon_cache_memory_shared_segment_shm *)calloc(1, segments_num * sizeof(phalcon_cache_memory_shared_segment_shm));
    ...
    *shared_segments_count = segments_num;
    ...
    for (i = 1; i < segments_num; i++) {
        ...
        if ((shared_segments[j].common.size - shared_segments[j].common.pos) >= (2 * PHALCON_CACHE_MEMORY_SMM_ALIGNED_SIZE(segment_size))) {
            (*shared_segments_p)[i].common.pos = 0;
            (*shared_segments_p)[i].common.size = PHALCON_CACHE_MEMORY_SMM_ALIGNED_SIZE(segment_size);
            (*shared_segments_p)[i].common.p = shared_segments[j].common.p + PHALCON_CACHE_MEMORY_SMM_ALIGNED_SIZE(shared_segments[j].common.pos);
            shared_segments[j].common.pos += PHALCON_CACHE_MEMORY_SMM_ALIGNED_SIZE(segment_size);
        } else {
            (*shared_segments_p)[i].common.pos = 0;
...
static int detach_segment(phalcon_cache_memory_shared_segment_shm *shared_segment) /* {{{ */ {
...
static size_t segment_type_size(void) /* {{{ */ {
	return sizeof(phalcon_cache_memory_shared_segment_shm);
}
/* }}} */

phalcon_cache_memory_shared_memory_handlers phalcon_cache_memory_alloc_shm_handlers = /* {{{ */ {
