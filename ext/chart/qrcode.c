...
static void _php_zbarcode_scan_page(zbar_image_scanner_t *scanner, zbar_image_t *image, zend_bool extended, zval *return_array)
{
	...
	for(; symbol; symbol = zbar_symbol_next(symbol)) {
        ...
        if (phalcon_function_exists_ex(SL("mb_convert_encoding")) == SUCCESS) {
			...
			phalcon_array_update_str(symbol_array, SL("data"), &totext, PH_COPY | PH_SEPARATE);                
        } else {
			phalcon_array_update_str_str(symbol_array, SL("data"), (char *)data, strlen(data), PH_COPY | PH_SEPARATE);
		}
		phalcon_array_update_str_str(symbol_array, SL("type"), (char *)type, strlen(type), PH_COPY | PH_SEPARATE);
		...
		if (extended) {
			...
			phalcon_array_update_str(symbol_array, SL("location"), loc_array, PH_COPY | PH_SEPARATE);
		}
