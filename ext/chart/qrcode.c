...
static int phalcon_list_qrcode;
static int dpi = 72;
...
PHALCON_INIT_CLASS(Phalcon_Chart_QRcode){
...
#ifdef PHALCON_USE_QRENCODE
    phalcon_list_qrcode = zend_register_list_destructors_ex(qr_dtor, NULL, "qrcode", module_number);
	...
	zend_declare_property_string(phalcon_chart_qrcode_ce, SL("_text"), "", ZEND_ACC_PROTECTED);
	zend_declare_property_long(phalcon_chart_qrcode_ce, SL("_version"), 0, ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Chart_QRcode, __construct){
	...
	PHALCON_THROW_EXCEPTION_STRW(phalcon_chart_exception_ce, "libpng is not installed, check your configuration");
	return;
#endif
#ifndef PHALCON_USE_QRENCODE
	PHALCON_THROW_EXCEPTION_STRW(phalcon_chart_exception_ce, "libqrencode is not installed, check your configuration");
	return;
#endif
}
...
PHP_METHOD(Phalcon_Chart_QRcode, render){
	...
	phalcon_return_property(&zid, getThis(), SL("_qr"));

	if (Z_TYPE(zid) == IS_NULL) {
		RETURN_FALSE;
	}

	qr = zend_fetch_resource(Z_RES(zid), "qr handle", phalcon_list_qrcode);

	fp = php_open_temporary_file(NULL, NULL, &path);
	if (!fp) {
		PHALCON_THROW_EXCEPTION_STRW(phalcon_chart_exception_ce, "Unable to open temporary file for writing");
		return;
	}
	...
	if(png_ptr == NULL) {
		fclose(fp);
		PHALCON_THROW_EXCEPTION_STRW(phalcon_chart_exception_ce, "Failed to initialize PNG writer");
		return;
	}
...
PHP_METHOD(Phalcon_Chart_QRcode, save){
	...
	if (Z_TYPE_P(filename) != IS_STRING || PHALCON_IS_EMPTY(filename)) {
		PHALCON_THROW_EXCEPTION_STRW(phalcon_chart_exception_ce, "filename parameter must be string");
		return;
	}
