...
static const zend_function_entry phalcon_di_factorydefault_cli_method_entry[] = {
	PHP_ME(Phalcon_Di_FactoryDefault_Cli, __construct, arginfo_empty, ZEND_ACC_PUBLIC|ZEND_ACC_CTOR)
	PHP_FE_END
};
...
PHALCON_INIT_CLASS(Phalcon_Di_FactoryDefault_Cli){
	...
	PHALCON_REGISTER_CLASS_EX(Phalcon\\Di\\FactoryDefault, Cli, di_factorydefault_cli, phalcon_di_ce, phalcon_di_factorydefault_cli_method_entry, 0);
...
PHP_METHOD(Phalcon_Di_FactoryDefault_Cli, __construct){
	...
	if (name) {
		PHALCON_CALL_PARENT(NULL, phalcon_di_factorydefault_cli_ce, getThis(), "__construct", name);
	} else {
