...
PHP_METHOD(Phalcon_Forms_Element_Password, __construct);
...
static const zend_function_entry phalcon_forms_element_password_method_entry[] = {
	PHP_ME(Phalcon_Forms_Element_Password, __construct, arginfo_phalcon_forms_elementinterface___construct, ZEND_ACC_PUBLIC|ZEND_ACC_CTOR)
...
PHALCON_INIT_CLASS(Phalcon_Forms_Element_Password){
	...
	zend_class_implements(phalcon_forms_element_password_ce, 1, phalcon_forms_elementinterface_ce);
...
PHP_METHOD(Phalcon_Forms_Element_Password, __construct){
	...
	if (!_type || PHALCON_IS_EMPTY(_type)) {
		PHALCON_STR(&type, "password");
	} else {
		PHALCON_CPY_WRT(&type, _type);
	}

	PHALCON_CALL_PARENTW(NULL, phalcon_forms_element_password_ce, getThis(), "__construct", name, attributes, options, options_values, &type);
}
