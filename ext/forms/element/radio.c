...
PHP_METHOD(Phalcon_Forms_Element_Radio, __construct);
PHP_METHOD(Phalcon_Forms_Element_Radio, render);

static const zend_function_entry phalcon_forms_element_radio_method_entry[] = {
	PHP_ME(Phalcon_Forms_Element_Radio, __construct, arginfo_phalcon_forms_elementinterface___construct, ZEND_ACC_PUBLIC|ZEND_ACC_CTOR)
...
PHP_METHOD(Phalcon_Forms_Element_Radio, __construct){
	...
	if (!_type || PHALCON_IS_EMPTY(_type)) {
		PHALCON_STR(&type, "radio");
	} else {
		PHALCON_CPY_WRT(&type, _type);
	}

	PHALCON_CALL_PARENTW(NULL, phalcon_forms_element_radio_ce, getThis(), "__construct", name, attributes, options, options_values, &type);
}
