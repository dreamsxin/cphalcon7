...
static const zend_function_entry phalcon_forms_manager_method_entry[] = {
	PHP_ME(Phalcon_Forms_Manager, __construct, arginfo_empty, ZEND_ACC_PUBLIC | ZEND_ACC_CTOR)
...
static zval* phalcon_forms_manager_read_dimension(zend_object *obj, zval *offset, int type, zval *rv)
{
	...
	phalcon_read_property(&forms, &object, SL("_forms"), PH_NOISY|PH_READONLY);
	...
	res = phalcon_hash_get(Z_ARRVAL(forms), offset, type);
...
PHP_METHOD(Phalcon_Forms_Manager, __construct)
{
	phalcon_update_property_empty_array(getThis(), SL("_forms"));
...
PHP_METHOD(Phalcon_Forms_Manager, get){
	...
	phalcon_read_property(&forms, getThis(), SL("_forms"), PH_NOISY|PH_READONLY);
	if (!phalcon_array_isset_fetch(&form, &forms, name, PH_READONLY)) {
