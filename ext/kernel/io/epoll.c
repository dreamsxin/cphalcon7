...
phalcon_io_callback_t phalcon_io_event_loop(void *tpi)
{
	...
	while (!pi->stop) {
		...
		for (ievent = 0; ievent < nevents; ievent++) {
			...
			if (ci==NULL)
				break;
			phalcon_io_debug_message(PHALCON_IO_DEBUG_IO, "event socket=%d, ci=%6x, ev=%s, op=%s\n",
...
void phalcon_io_add_server_to_poll (phalcon_io_client_info *ci) {
	phalcon_io_change_poll (ci, EPOLL_CTL_ADD, EPOLLIN);
}
...
void phalcon_io_add_client_to_poll (phalcon_io_client_info *ci) {
	...
	if (ci->use_write_events)
		phalcon_io_change_poll (ci, EPOLL_CTL_ADD, EPOLLIN | EPOLLOUT | EPOLLET);
	else
		phalcon_io_change_poll (ci, EPOLL_CTL_ADD, EPOLLIN | EPOLLET);
}

void phalcon_io_remove_client_from_poll (phalcon_io_client_info *ci) {
	((phalcon_io_poller_info *)ci->tpi)->actual_connections--;
	phalcon_io_change_poll (ci, EPOLL_CTL_DEL, 0);
}
...
void phalcon_io_change_poll (phalcon_io_client_info *ci, int operation, int events)
{
	...
	phalcon_io_poll_event pe;
  	phalcon_io_debug_message(PHALCON_IO_DEBUG_IO, "phalcon_io_change_poll socket=%d, ci=%6x, ev=%s, op=%s\n",
  		ci->socket, ci, phalcon_io_name_event(events), phalcon_io_name_operation(operation));
	pe.events = events;
	pe.data.ptr = ci;
	int ret = epoll_ctl (((phalcon_io_poller_info *)ci->tpi)->poll_handler, operation, ci->socket, &pe);
	if (ret < 0)
