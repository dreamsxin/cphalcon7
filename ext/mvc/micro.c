...
PHP_METHOD(Phalcon_Mvc_Micro, handle){
	...
	if (Z_TYPE_P(matched_route) == IS_OBJECT) {
		...
		handlers = phalcon_read_property(this_ptr, SL("_handlers"), PH_NOISY);
		...
		if (PHALCON_IS_FALSE(status)) {
			...
			handler = phalcon_read_property(this_ptr, SL("_activeHandler"), PH_NOISY);
		}

		before_handlers = phalcon_read_property(this_ptr, SL("_beforeHandlers"), PH_NOISY);
			...
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(before_handlers), before) {
				if (Z_TYPE_P(before) == IS_OBJECT) {
					...
					if (is_middleware) {
						 ...
						 * Reload the status
						 */
						stopped = phalcon_read_property(this_ptr, SL("_stopped"), PH_NOISY);
		...
		after_handlers = phalcon_read_property(this_ptr, SL("_afterHandlers"), PH_NOISY);
		 ...
		 */
		not_found_handler = phalcon_read_property(this_ptr, SL("_notFoundHandler"), PH_NOISY);
	...
	finish_handlers = phalcon_read_property(this_ptr, SL("_finishHandlers"), PH_NOISY);
...
PHP_METHOD(Phalcon_Mvc_Micro, _throwException){
	...
	handler = phalcon_read_property(this_ptr, SL("_errorHandler"), PH_NOISY);
