...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, getOption){
	...
	if (phalcon_array_isset_fetch(return_value, options, option)) {
		return;
	}
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(extensions), extension) {
			...
			if (phalcon_method_exists(extension, name) == SUCCESS) {
				...
				if (Z_TYPE_P(arguments) == IS_ARRAY) { 
					...
					PHALCON_CALL_USER_FUNCW(return_value, &call_object);
				}

				PHALCON_PTR_DTOR(&call_object);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, addExtension){
	...
	if (phalcon_method_exists_ex(extension, SL("initialize")) == SUCCESS) {
		PHALCON_CALL_METHODW(NULL, extension, "initialize", getThis());
	}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, getUniquePrefix){
	...
	if (Z_TYPE(prefix) == IS_OBJECT) {
		if (instanceof_function(Z_OBJCE(prefix), zend_ce_closure)) {
			...
			PHALCON_CALL_FUNCTIONW(&calculated_prefix, "call_user_func_array", &prefix, &parameters);
			PHALCON_PTR_DTOR(&parameters);
	...
	if (Z_TYPE(prefix) != IS_STRING) {
		PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "The unique compilation prefix is invalid");
		return;
	}

	RETURN_CTOR_DTORW(&prefix);
}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, attributeReader){
	...
	if (PHALCON_IS_LONG(&left_type, PHVOLT_T_IDENTIFIER)) {
		...
		if (PHALCON_IS_STRING(&variable, "loop")) {
			...
			PHALCON_CALL_METHODW(&prefix, getThis(), "getuniqueprefix");
			...
			if (Z_TYPE_P(dependency_injector) == IS_OBJECT) {
				PHALCON_CALL_METHODW(&is_service, dependency_injector, "has", &variable);
		...
		PHALCON_PTR_DTOR(&variable);
	} else {
		PHALCON_CALL_METHODW(&left_code, getThis(), "expression", &left);
	...
	PHALCON_PTR_DTOR(&right);

	RETURN_CTOR_DTORW(&expr_code);
}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, functionCall){
	...
	if (Z_TYPE_P(expr) != IS_ARRAY) { 
		PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "Corrupted function call");
		return;
	}

	if (phalcon_array_isset_fetch_str(&func_arguments, expr, SL("arguments"))) {
		PHALCON_CALL_METHODW(&arguments, getThis(), "expression", &func_arguments);
	}
	...
	if (PHALCON_IS_LONG(&name_type, PHVOLT_T_IDENTIFIER)) {
		...
		if (Z_TYPE_P(extensions) == IS_ARRAY) {
			...
			PHALCON_CALL_METHODW(&code, getThis(), "fireextensionevent", &event, &fire_arguments);
			PHALCON_PTR_DTOR(&fire_arguments);
			...
			if (phalcon_array_isset_fetch(&definition, &functions, &name)) {
				...
				if (Z_TYPE(definition) == IS_OBJECT) {
					if (instanceof_function(Z_OBJCE(definition), zend_ce_closure)) {
						...
						PHALCON_RETURN_CALL_FUNCTIONW("call_user_func_array", &definition, &parameters);
						PHALCON_PTR_DTOR(&parameters);
						return;
					}
				}
			...
			if (Z_TYPE_P(extended_blocks) == IS_ARRAY) { 
				...
				if (phalcon_array_isset(extended_blocks, current_block)) {
					...
					if (Z_TYPE(block) == IS_ARRAY) { 
						PHALCON_CALL_METHODW(&code, getThis(), "_statementlistorextends", &block);
		...
		if (phalcon_method_exists(&class_name, &method) == SUCCESS) {
			...
			PHALCON_PTR_DTOR(&array_helpers);
		...
		if (PHALCON_IS_STRING(&name, "version")) {
			RETURN_STRING("Phalcon\\Version::get()");
		}

		if (PHALCON_IS_STRING(&name, "version_id")) {
			RETURN_STRING("Phalcon\\Version::getId()");
		}
	...
	PHALCON_CALL_METHODW(&name, getThis(), "expression", &name_expr);
	PHALCON_PTR_DTOR(&name_expr);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, resolveTest){
	...
	phalcon_fetch_params(0, 2, 0, &test, &left);

	/** 
	 * Valid tests are always arrays
	 */
	if (Z_TYPE_P(test) != IS_ARRAY) { 
		PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "Corrupted test");
		return;
	}
	...
	if (PHALCON_IS_LONG(&type, PHVOLT_T_FCALL)) {
		...
		if (phalcon_array_isset_fetch_str(&name, &test_name, SL("value"))) {
			...
			if (PHALCON_IS_STRING(&name, "divisibleby")) {
				...
				PHALCON_CALL_METHODW(&arguments, getThis(), "expression", &test_arguments);
	 ...
	 */
	PHALCON_CALL_METHODW(&right_code, getThis(), "expression", test);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, resolveFilter){
	...
	if (Z_TYPE_P(filter) != IS_ARRAY) { 
		PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "Corrupted filter");
		return;
	}
	 ...
	 * Resolve arguments
	 */
	if (phalcon_array_isset_fetch_str(&func_arguments, filter, SL("arguments"))) {
		phalcon_array_fetch_str(&file, filter, SL("file"), PH_NOISY);
		phalcon_array_fetch_str(&line, filter, SL("line"), PH_NOISY);
		...
		if (!PHALCON_IS_STRING(&name, "default")) {
			...
			PHALCON_PTR_DTOR(&resolved_expr);

			ZVAL_MAKE_REF(&func_arguments);
			PHALCON_CALL_FUNCTIONW(NULL, "array_unshift", &func_arguments, &resolved_param);
			ZVAL_UNREF(&func_arguments);

			PHALCON_PTR_DTOR(&resolved_param);
		}

		PHALCON_CALL_METHODW(&arguments, getThis(), "expression", &func_arguments);
		PHALCON_PTR_DTOR(&func_arguments);
	} else {
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, expression){
	 ...
	 */
	if (Z_TYPE_P(expr) != IS_ARRAY) { 
		PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "Corrupted expression");
		return;
	}
	...
	while (1) {
		zval event = {}, fire_arguments = {}, type = {}, items = {}, *single_expr, left = {}, left_code = {}, right_code = {}, right = {}, value = {};
		zval single_quote = {}, escaped_quoute = {}, escaped_string = {}, start_code = {}, end_code = {}, ternary = {};
		zval ternary_code = {}, line = {}, file = {}, exception_message = {};
		...
		if (Z_TYPE_P(extensions) == IS_ARRAY) { 
			...
			PHALCON_CALL_METHODW(&expr_code, getThis(), "fireextensionevent", &event, &fire_arguments);
			PHALCON_PTR_DTOR(&fire_arguments);
		...
		if (!phalcon_array_isset_fetch_str(&type, expr, SL("type"))) {
			...
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(expr), single_expr) {
				...
				PHALCON_CALL_METHODW(&single_expr_code, getThis(), "expression", &single_expr_expr);
				PHALCON_PTR_DTOR(&single_expr_expr);
				...
				PHALCON_PTR_DTOR(&single_expr_code);
			} ZEND_HASH_FOREACH_END();
		...
		if (PHALCON_IS_LONG(&type, PHVOLT_T_DOT)) {
			PHALCON_CALL_METHODW(&expr_code, getThis(), "attributereader", expr);
			break;
		}
		...
		if (PHALCON_IS_LONG(&type, PHVOLT_T_IS)) {
			...
			PHALCON_CALL_METHODW(&expr_code, getThis(), "resolvetest", &right_code, &left_code);
		} else if (PHALCON_IS_LONG(&type, PHVOLT_T_PIPE)) {
			phalcon_array_fetch_str(&right_code, expr, SL("right"), PH_NOISY);
		...
		switch (phalcon_get_intval(&type)) {
			...
			default:
				...
				PHALCON_THROW_EXCEPTION_ZVALW(phalcon_mvc_view_exception_ce, &exception_message);
				PHALCON_PTR_DTOR(&exception_message);
				PHALCON_PTR_DTOR(&file);
				return;
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, compileForeach){
	...
	zval *statement, *extends_mode = NULL, expr = {}, prefix = {}, *level, compilation = {}, prefix_level = {}, expr_code = {}, block_statements = {}, for_else = {}, *bstatement;
	...
	if (phalcon_array_isset_fetch_str(&if_expr, statement, SL("if_expr"))) {
		PHALCON_CALL_METHODW(&if_expr_code, getThis(), "expression", &if_expr);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, compileCache){
	...
	zval *statement, *extends_mode = NULL, compilation = {}, expr = {}, expr_code = {}, lifetime = {}, lifetime_type = {}, lifetime_value = {}, block_statements = {}, code = {};
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, compileInclude){
	 ...
	 */
	if (PHALCON_IS_LONG(&expr_type, PHVOLT_T_STRING)) {
		/** 
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, compileAutoEscape){
	...
	zval *statement, *extends_mode, *old_autoescape, autoescape = {}, block_statements = {};

	phalcon_fetch_params(0, 2, 0, &statement, &extends_mode);
	...
	PHALCON_RETURN_CALL_METHODW(getThis(), "_statementlist", &block_statements, extends_mode);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, compileMacro){
	...
	zval *statement, *extends_mode, name = {}, *macros, exception_message = {}, code = {}, parameters = {};
	zval *parameter, block_statements = {}, block_code = {};
	zend_string *str_key;
	ulong idx;
	...
	if (!phalcon_array_isset_str(statement, SL("parameters"))) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL(parameters), idx, str_key, parameter) {
			zval position = {}, variable_name = {};
			if (str_key) {
	...
	if (phalcon_array_isset_str(statement, SL("block_statements"))) {
		 ...
		 * Process statements block
		 */
		PHALCON_CALL_METHODW(&block_code, getThis(), "_statementlist", &block_statements, extends_mode);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, _statementList){
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(statements), statement) {
		...
		switch (phalcon_get_intval(&type)) {
			...
			case PHVOLT_T_IF:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compileif", statement, extends_mode);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;

			case PHVOLT_T_ELSEIF:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compileelseif", statement);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;

			case PHVOLT_T_FOR:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compileforeach", statement, extends_mode);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;

			case PHVOLT_T_SET:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compileset", statement);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;

			case PHVOLT_T_ECHO:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compileecho", statement);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;
			...
			case PHVOLT_T_EXTENDS:
				...
				PHALCON_CALL_METHODW(&temp_compilation, &sub_compiler, "compile", &final_path, &extended);

				/** 
				 * If the compilation doesn't return anything we include the compiled path
				 */
				if (Z_TYPE(temp_compilation) == IS_NULL) {
					PHALCON_CALL_METHODW(&compiled_path, &sub_compiler, "getcompiledtemplatepath");
			...
			case PHVOLT_T_INCLUDE:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compileinclude", statement);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;

			case PHVOLT_T_CACHE:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compilecache", statement, extends_mode);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;

			case PHVOLT_T_DO:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compiledo", statement);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;

			case PHVOLT_T_RETURN:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compilereturn", statement);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;

			case PHVOLT_T_AUTOESCAPE:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compileautoescape", statement, extends_mode);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;
			...
			case PHVOLT_T_ELSEFOR:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compileforelse");
				phalcon_concat_self(&compilation, &temp_compilation);
				break;

			case PHVOLT_T_MACRO:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compilemacro", statement, extends_mode);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;

			case PHVOLT_T_CALL:
				PHALCON_CALL_METHODW(&temp_compilation, getThis(), "compilecall", statement, extends_mode);
				phalcon_concat_self(&compilation, &temp_compilation);
				break;

			case PHVOLT_T_EMPTY_STATEMENT:
				break;
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, _compileSource){
	...
	if (Z_TYPE(intermediate) == IS_ARRAY) {
		...
		if (PHALCON_IS_TRUE(extended)) {
				...
				if (str_key) {
					...
					if (Z_TYPE_P(block) == IS_ARRAY) { 
						if (phalcon_array_isset_fetch(&local_block, blocks, &tmp)) {
							...
							PHALCON_PTR_DTOR(&local_block);
						} else {
							/** 
							 * The block is not set local only in the extended template
							 */
							PHALCON_CALL_METHODW(&block_compilation, getThis(), "_statementlist", block);
						}
					} else {
						if (phalcon_array_isset_fetch(&local_block, blocks, &tmp)) {
							/** 
			...
			RETURN_CTOR_DTORW(&final_compilation);
		}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, compileFile){
	...
	PHALCON_PTR_DTOR(&final_compilation);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, compile){
	...
	zval *template_path, *extends_mode = NULL, stat = {}, compile_always = {}, compiled_path = {}, prefix = {}, compiled_separator = {}, compiled_extension = {};
	zval compilation = {}, *options, real_template_path = {}, template_sep_path = {}, compiled_template_path = {};
	zval params = {}, real_compiled_path = {}, blocks_code = {}, exception_message = {};

	phalcon_fetch_params(0, 1, 1, &template_path, &extends_mode);
	...
	if (Z_TYPE_P(options) == IS_ARRAY) {
		 ...
		 */
		if (phalcon_array_isset_fetch_str(&compile_always, options, SL("compileAlways"))) {
			if (!PHALCON_IS_BOOL(&compile_always)) {
				PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "compileAlways must be a bool value");
				return;
			}
		}
		...
		if (phalcon_array_isset_fetch_str(&prefix, options, SL("prefix"))) {
			if (Z_TYPE(prefix) != IS_STRING) {
				PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "prefix must be a string");
				return;
			}
		}
		 ...
		 */
		if (phalcon_array_isset_fetch_str(&compiled_path, options, SL("compiledPath"))) {
			if (Z_TYPE(compiled_path) != IS_STRING) {
				if (Z_TYPE(compiled_path) != IS_OBJECT) {
					PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "compiledPath must be a string or a closure");
					return;
				}
			}
		}
		 ...
		 */
		if (phalcon_array_isset_fetch_str(&compiled_separator, options, SL("compiledSeparator"))) {
			if (Z_TYPE(compiled_separator) != IS_STRING) {
				PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "compiledSeparator must be a string");
				return;
			}
		}
		 ...
		 */
		if (phalcon_array_isset_fetch_str(&compiled_extension, options, SL("compiledExtension"))) {
			if (Z_TYPE(compiled_extension) != IS_STRING) {
				PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "compiledExtension must be a string");
				return;
			}
		}
	...
	if (Z_TYPE(compiled_path) == IS_STRING) {
		...
		PHALCON_PTR_DTOR(&template_sep_path);
	} else {
		/** 
		 * A closure can dynamically compile the path
		 */
		if (Z_TYPE(compiled_path) == IS_OBJECT) {
			if (instanceof_function(Z_OBJCE(compiled_path), zend_ce_closure)) {
				...
				PHALCON_CALL_USER_FUNC_ARRAYW(&compiled_template_path, &compiled_path, &params);
				PHALCON_PTR_DTOR(&params);

				/** 
				 * The closure must return a valid path
				 */
				if (Z_TYPE(compiled_template_path) != IS_STRING) {
					PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "compiledPath closure didn't return a valid string");
					return;
				}
			} else {
				PHALCON_THROW_EXCEPTION_STRW(phalcon_mvc_view_exception_ce, "compiledPath must be a string or a closure");
	...
	PHALCON_PTR_DTOR(&compiled_path);
	PHALCON_PTR_DTOR(&compiled_extension);
	PHALCON_PTR_DTOR(&compiled_separator);
	...
	if (zend_is_true(&compile_always)) {
		 ...
		 */
		PHALCON_CALL_METHODW(&compilation, getThis(), "compilefile", template_path, &real_compiled_path, extends_mode);
	} else {
				...
				if (phalcon_compare_mtime(template_path, &real_compiled_path)) {
					...
					if (PHALCON_IS_TRUE(extends_mode)) {
						...
						if (PHALCON_IS_FALSE(&blocks_code)) {
							...
							PHALCON_THROW_EXCEPTION_ZVALW(phalcon_mvc_view_exception_ce, &exception_message);
							PHALCON_PTR_DTOR(&exception_message);
							PHALCON_PTR_DTOR(&real_compiled_path);
							return;
						}
	...
	PHALCON_PTR_DTOR(&compiled_template_path);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, parse){
	...
	phalcon_fetch_params(0, 1, 0, &view_code);
