...
static PHP_METHOD(Psr_Log_LoggerAwareTrait, setLogger)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS(), "Z", &logger)) {
		RETURN_NULL();
	}

	zend_update_property(psr_log_loggerawaretrait_ce, getThis(), ZEND_STRL("logger"), *logger);
}
...
PHALCON_INIT_CLASS(Psr_Log_LoggerAwareTrait)
{
	...
	psr_log_loggerawaretrait_ce = zend_register_internal_class(&ce);
	if (EXPECTED(psr_log_loggerawaretrait_ce != NULL)) {
		psr_log_loggerawaretrait_ce->ce_flags |= ZEND_ACC_TRAIT;

		zend_declare_property_null(psr_log_loggerawaretrait_ce, ZEND_STRL("logger"), ZEND_ACC_PROTECTED);
		return SUCCESS;
	}
