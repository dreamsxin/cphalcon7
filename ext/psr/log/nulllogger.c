...
static PHP_METHOD(Psr_Log_NullLogger, log)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS(), "zz|z!", &level, &message, &context)) {
		RETURN_NULL();
	}
}
...
PHALCON_INIT_CLASS(Psr_Log_NullLogger)
{
	...
	psr_log_nulllogger_ce = zend_register_internal_class_ex(&ce, psr_log_abstractlogger_ce, NULL);
