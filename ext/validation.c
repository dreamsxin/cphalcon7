...
int phalcon_validation_getdefaultmessage_helper(const zend_class_entry *ce, zval **return_value_ptr, zval *this_ptr, const char *type)
{
...

	{
		...
		return phalcon_return_call_method(*return_value_ptr, NULL, this_ptr, "getdefaultmessage", 1, params);
	}
}
...
PHP_METHOD(Phalcon_Validation, __construct){
	...
	if (Z_TYPE_P(validators) != IS_NULL) {
		...
		phalcon_update_property_this(this_ptr, SL("_validators"), validators);
	}
	
	PHALCON_CALL_METHODW(NULL, getThis(), "setdefaultmessages");

	/* Check for an 'initialize' method */
	if (phalcon_method_exists_ex(this_ptr, SS("initialize")) == SUCCESS) {
...
PHP_METHOD(Phalcon_Validation, validate){
	 ...
	 */
	phalcon_update_property_null(this_ptr, SL("_values"));
	 ...
	 */
	if (phalcon_method_exists_ex(this_ptr, SS("beforevalidation")) == SUCCESS) {
	...
	phalcon_update_property_this(this_ptr, SL("_messages"), messages);
	...
	if (phalcon_method_exists_ex(this_ptr, SS("aftervalidation")) == SUCCESS) {
...
PHP_METHOD(Phalcon_Validation, setFilters){
	...
	phalcon_update_property_array(this_ptr, SL("_filters"), attribute, filters);
	RETURN_THISW();
}
...
PHP_METHOD(Phalcon_Validation, bind){
	...
	phalcon_update_property_this(this_ptr, SL("_entity"), entity);
	phalcon_update_property_this(this_ptr, SL("_data"), data);
...
PHP_METHOD(Phalcon_Validation, getValue){
	...
	phalcon_read_property_this(&entity, this_ptr, SL("_entity"), PH_NOISY);
	...
	if (Z_TYPE_P(entity) == IS_OBJECT) {
		...
		} else if (phalcon_method_exists_ex(entity, SS("readattribute")) == SUCCESS) {
			PHALCON_CALL_METHOD(&value, entity, "readattribute", attribute);
		} else if (phalcon_isset_property_zval(entity, attribute)) {
			PHALCON_OBS_VAR(value);
			phalcon_read_property_zval(&value, entity, attribute, PH_NOISY);
		} else {
	...
	phalcon_read_property_this(&data, this_ptr, SL("_data"), PH_NOISY);
	if (Z_TYPE_P(data) != IS_ARRAY) { 
		if (Z_TYPE_P(data) != IS_OBJECT) {
	...
	phalcon_read_property_this(&values, this_ptr, SL("_values"), PH_NOISY);
	...
	if (Z_TYPE_P(data) == IS_ARRAY) { 
		...
		if (Z_TYPE_P(data) == IS_OBJECT) {
			if (phalcon_isset_property_zval(data, attribute)) {
				PHALCON_OBS_NVAR(value);
				phalcon_read_property_zval(&value, data, attribute, PH_NOISY);
			}
		}
	}
	...
	if (Z_TYPE_P(value) != IS_NULL) {
		 ...
		 */
		phalcon_update_property_array(this_ptr, SL("_values"), attribute, value);
...
PHP_METHOD(Phalcon_Validation, setDefaultMessages)
{
	...
	add_assoc_stringl_ex(default_messages, SS("Alnum"),             SL("Field :field must contain only letters and numbers"));
	add_assoc_stringl_ex(default_messages, SS("Alpha"),             SL("Field :field must contain only letters"));
	add_assoc_stringl_ex(default_messages, SS("Between"),           SL("Field :field must be within the range of :min to :max"));
	add_assoc_stringl_ex(default_messages, SS("Confirmation"),      SL("Field :field must be the same as :with"));
	add_assoc_stringl_ex(default_messages, SS("Digit"),             SL("Field :field must be numeric"));
	add_assoc_stringl_ex(default_messages, SS("Email"),             SL("Field :field must be an email address"));
	add_assoc_stringl_ex(default_messages, SS("ExclusionIn"),       SL("Field :field must not be a part of list: :domain"));
	add_assoc_stringl_ex(default_messages, SS("FileEmpty"),         SL("Field :field must not be empty"));
	add_assoc_stringl_ex(default_messages, SS("FileIniSize"),       SL("File :field exceeds the maximum file size"));
	add_assoc_stringl_ex(default_messages, SS("FileMaxResolution"), SL("File :field must not exceed :max resolution"));
	add_assoc_stringl_ex(default_messages, SS("FileMinResolution"), SL("File :field must be at least :min resolution"));
	add_assoc_stringl_ex(default_messages, SS("FileSize"),          SL("File :field exceeds the size of :max"));
	add_assoc_stringl_ex(default_messages, SS("FileMaxSize"),       SL("File :field the size must not exceed :max"));
	add_assoc_stringl_ex(default_messages, SS("FileMinSize"),       SL("File :field the size must be at least :min"));
	add_assoc_stringl_ex(default_messages, SS("FileType"),          SL("File :field must be of type: :types"));
	add_assoc_stringl_ex(default_messages, SS("FileValid"),         SL("Field :field is not valid"));
	add_assoc_stringl_ex(default_messages, SS("ImageMaxWidth"),     SL("Image :field the width must not exceed :max"));
	add_assoc_stringl_ex(default_messages, SS("ImageMinWidth"),     SL("Image :field the width must be at least :min"));
	add_assoc_stringl_ex(default_messages, SS("ImageMaxHeight"),    SL("Image :field the height must not exceed :max"));
	add_assoc_stringl_ex(default_messages, SS("ImageMinHeight"),    SL("Image :field the height must be at least :min"));
	add_assoc_stringl_ex(default_messages, SS("Identical"),         SL("Field :field does not have the expected value"));
	add_assoc_stringl_ex(default_messages, SS("InclusionIn"),       SL("Field :field must be a part of list: :domain"));
	add_assoc_stringl_ex(default_messages, SS("PresenceOf"),        SL("Field :field is required"));
	add_assoc_stringl_ex(default_messages, SS("Regex"),             SL("Field :field does not match the required format"));
	add_assoc_stringl_ex(default_messages, SS("TooLong"),           SL("Field :field must not exceed :max characters long"));
	add_assoc_stringl_ex(default_messages, SS("TooShort"),          SL("Field :field must be at least :min characters long"));
	add_assoc_stringl_ex(default_messages, SS("Uniqueness"),        SL("Field :field must be unique"));
	add_assoc_stringl_ex(default_messages, SS("Url"),               SL("Field :field must be a url"));
...
PHP_METHOD(Phalcon_Validation, setLabels) {
	...
	phalcon_update_property_this(this_ptr, SL("_labels"), labels);
}
