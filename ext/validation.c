...
static const zend_function_entry phalcon_validation_method_entry[] = {
	...
	PHP_ME(Phalcon_Validation, getMessage, arginfo_phalcon_validationinterface_getmessage, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_FE_END
};
...
PHP_METHOD(Phalcon_Validation, getLabel) {
	...
	if (Z_TYPE_P(field_param) == IS_ARRAY) {
		...
		if (Z_TYPE(labels) == IS_ARRAY) {
			PHALCON_CALL_CE_STATIC(&label_values, phalcon_arr_ce, "get", field_param);
			if (PHALCON_IS_NOT_EMPTY(&label_values)) {
				if (Z_TYPE(label_values) == IS_ARRAY) {
					phalcon_fast_join_str(&value, SL(", "), &label_values);
				} else {
					PHALCON_CPY_WRT(&value, &label_values);
				}
goto end;
			}
		}
		...
		if (Z_TYPE(entity) == IS_OBJECT && phalcon_method_exists_ex(&entity, SL("getlabel")) == SUCCESS) {
			PHALCON_CALL_METHOD(&label_values, &entity, "getlabel", field_param);
			if (PHALCON_IS_NOT_EMPTY(&label_values)) {
				if (Z_TYPE(label_values) == IS_ARRAY) {
					phalcon_fast_join_str(&value, SL(", "), &label_values);
				} else {
