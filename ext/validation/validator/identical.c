...
PHP_METHOD(Phalcon_Validation_Validator_Identical, validate){
	...
	zval 
	zval label, pairs, message_str, code, *prepared = NULL, *message;
	...
	if (PHALCON_IS_FALSE(valid)) {
		...
		if (!zend_is_true(message_str)) {
			RETURN_MM_ON_FAILURE(phalcon_validation_getdefaultmessage_helper(&message_str, Z_OBJCE_P(validator), validator, "Identical"));
		}
