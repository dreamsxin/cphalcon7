...
static const zend_function_entry phalcon_validation_validator_json_method_entry[] = {
	...
	PHP_ME(Phalcon_Validation_Validator_Json, valid, arginfo_phalcon_validation_validatorinterface_valid, ZEND_ACC_PUBLIC)
	PHP_FE_END
};
...
PHP_METHOD(Phalcon_Validation_Validator_Json, validate){
	...
	if (PHALCON_IS_FALSE(&valid)) {
		...
		if (!zend_is_true(&message_str)) {
			RETURN_ON_FAILURE(phalcon_validation_getdefaultmessage_helper(&message_str, Z_OBJCE_P(validaton), validaton, "Json"));
		}
