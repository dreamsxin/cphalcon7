...
class ModelsDynamicOperationsTest extends PHPUnit_Framework_TestCase
{
	...
	protected function _executeTestsNormal($di, &$tracer)
	{
		...
		$this->assertEquals('UPDATE `personas` SET `nombres` = :nombres WHERE `personas`.`cedula` = :phi_cedula', $tracer[3]);
		...
		$this->assertEquals('UPDATE `personas` SET `nombres` = :nombres, `direccion` = :direccion WHERE `personas`.`cedula` = :phi_cedula', $tracer[4]);
	}
	...
	protected function _executeTestsRenamed($di, &$tracer)
	{
		...
		$this->assertEquals('UPDATE `personas` SET `nombres` = :navnes WHERE `personas`.`cedula` = :phi_borgerId', $tracer[3]);
		...
		$this->assertEquals('UPDATE `personas` SET `nombres` = :navnes, `direccion` = :adresse WHERE `personas`.`cedula` = :phi_borgerId', $tracer[4]);
	}
